...
import { BUSINESS_MODELS } from "@/lib/finance/business-models";
...
function renderText(text: string) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("### ")) {
      elements.push(
        <h3 key={i} className="text-base font-bold text-[#212121] mt-4 mb-1">
          {line.slice(4)}
        </h3>
      );
    } else if (line.startsWith("## ")) {
      elements.push(
        <h2 key={i} className="text-lg font-bold text-[#212121] mt-5 mb-2">
          {line.slice(3)}
        </h2>
      );
    } else if (line.startsWith("# ")) {
      elements.push(
        <h1 key={i} className="text-xl font-bold text-[#212121] mt-5 mb-2">
          {line.slice(2)}
        </h1>
      );
    } else if (line.startsWith("|")) {
            ...
            {tableLines.map((row, ri) => {
              ...
              return (
                <tr key={ri} className={isHeader ? "bg-[#f5f5f5]" : "border-b border-[#e8e8e8]"}>
                  {cells.map((cell, ci) =>
                    isHeader ? (
                      <th key={ci} className="text-left px-3 py-2 font-semibold text-[#212121] border border-[#e8e8e8]">
                        {inlineFormat(cell)}
                      </th>
                    ) : (
                      <td key={ci} className="px-3 py-2 text-[#444] border border-[#e8e8e8]">
                        {inlineFormat(cell)}
                      </td>
                    )
                  )}
      ...
      elements.push(
        <ul key={`ul-${i}`} className="list-disc list-inside space-y-1 my-2 text-sm text-[#444]">
          {listItems.map((item, li) => (
            <li key={li}>{inlineFormat(item)}</li>
          ))}
        </ul>
      );
      continue;
...
function ModelCard({
  ...
  return (
    <div
      className="bg-white rounded-xl border border-[#e8e8e8] overflow-hidden cursor-pointer select-none"
    ...
    >
      ...
      <div className="p-4 space-y-3">
        <div className="flex items-start justify-between gap-2">
          <div>
            <div className="text-xs font-semibold text-[#717182] uppercase tracking-wide mb-0.5">
              {meta.label}
            </div>
            <div className="text-sm font-bold text-[#212121]">{model.name}</div>
          </div>
          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full shrink-0 ${meta.feasibilityColor}`}>
            {model.feasibility}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between">
          <span className={`inline-block text-xs font-medium px-2 py-0.5 rounded-full ${meta.phaseBg}`}>
            {meta.phaseLabel}
          </span>
          <button
            onClick={(e) => {
      ...
      {expanded && (
        <div className="border-t border-[#e8e8e8] bg-[#fafafa] p-4 space-y-4">
          {/* Unit economics */}
          <div>
            <p className="text-xs font-semibold text-[#717182] uppercase tracking-wide mb-2">
              Unit Economics
            </p>
                  ...
                  <tbody>
                    {model.projections.map((p) => (
                      <tr key={p.year} className="border-b border-[#e0e0e0]">
                        ...
                        <td className="px-2 py-1.5 text-right font-medium text-[#212121] border border-[#e0e0e0]">
                          €{p.mrr_eur.toLocaleString()}
                        </td>
                        <td className="px-2 py-1.5 text-right font-medium text-[#212121] border border-[#e0e0e0]">
                          €{p.arr_eur.toLocaleString()}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  ...
                  <div key={tier.name} className="bg-white border border-[#e8e8e8] rounded-lg p-2.5">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-xs text-[#717182]">
                        {tier.price_monthly_eur === 0
                          ? "Free"
                          : `€${tier.price_monthly_eur}/mo · €${tier.price_annual_eur}/yr`}
                      </span>
                    </div>
                    <ul className="space-y-0.5">
                      {tier.features.map((f, fi) => (
                        <li key={fi} className="text-xs text-[#717182]">
                          · {f}
                        </li>
                      ))}
                    </ul>
          ...
          <button
            onClick={() =>
              onAsk(
                `Explain step by step how you calculated the projections and unit economics for "${model.name}"`
              )
            }
            className="w-full text-xs bg-[#212121] text-white rounded-lg px-3 py-2 hover:bg-[#333] transition-colors"
          >
            Ask agent to walk through the full calculation
          </button>
        </div>
      )}
    </div>
    ...
    <div className="min-h-screen bg-[#f9f9f9]">
      ...
      <div className="bg-white border-b border-[#e8e8e8] px-6 py-4">
        ...
        <p className="text-sm text-[#717182] mt-0.5">
          AI-powered business model advisor · double-click any card to drill into calculations
        </p>
      </div>

      <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        {/* Model cards (shown until first message) */}
        {showCards && (
          <>
            ...
            <div>
              <p className="text-xs font-semibold text-[#717182] uppercase tracking-wide mb-3">
                Ask the advisor
              </p>
              ...
              >
                {msg.role === "user" ? (
                  ...
                  <div className="bg-white border border-[#e8e8e8] rounded-2xl rounded-tl-sm px-4 py-3 max-w-[85%] w-full">
                    <div className="text-xs font-semibold text-[#717182] mb-2 uppercase tracking-wide">
                      Finance Advisor
                    </div>
